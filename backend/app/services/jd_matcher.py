...
def generate_gap_feedback(resume_text: str, jd_text: str) -> list[str]:
    """
    ...
    Returns:
    - list[str]: GPT-generated suggestions for resume improvement
    """
    prompt = f"""
...
3. ...
    ...
    content = response.choices[0].message.content or "No suggestions generated."
    suggestions = re.findall(r'^\s*\d+\.\s*(.+?)(?=\n\s*\d+\.|\Z)', content, re.DOTALL | re.MULTILINE)
